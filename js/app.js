...
function renderLanding(){
  const cards = DATA.indications.map((ind, i) => {
    ...
    <a class="preset-card" href="#/indication/${ind.id}" style="--pc:${ind.accent}">
      <div class="idx">${n} CASE${n===1?"":"S"}</div>
      <h3>${esc(ind.name)}</h3>
    ...
    <main>
      <div class="wrap">
        <section class="hero">
          ...
          <h1>ECU Med Peds POCUS Curriculum</h1>
          <p class="lede">Pick a chief complaint, work through the history and exam like a real consult, then scan every window yourself on your standardized patient. Compare what you find on your patient with what is seen in the case.</p>
        </section>
...
function renderCase(indId, caseId){
  ...
  const caseNum = (DATA.cases[indId] || []).findIndex(c => c.id === kase.id) + 1;
    ...
    <main>
      <div class="wrap">
        <div class="case-header" style="--ind-accent:${ind.accent}">
          <h1 class="case-title-line">${esc(ind.name)} - Case ${caseNum}</h1>
        </div>

        <div class="panel" style="--ind-accent:${ind.accent}">
          <div class="panel-head"><h2>History of Present Illness</h2></div>
          <p class="hpi-text">${esc(kase.hpi)}</p>
        </div>

        <div class="panel" style="--ind-accent:${ind.accent}">
          <div class="panel-head"><h2>Vitals &amp; Physical Exam</h2></div>
          <div class="vitals-grid">${vitalsHtml}</div>
          <p class="exam-text">${esc(kase.exam)}</p>
        </div>

        <div class="panel" style="--ind-accent:${ind.accent}">
          <div class="panel-head"><h2>Relevant Labs</h2></div>
        ...
        <div class="panel" style="--ind-accent:${ind.accent}">
          <div class="panel-head"><h2>Ultrasound Findings</h2></div>
  ...
  document.querySelectorAll(".view-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      const stageInner = finding.gif
        ...
        : `<div class="clip-placeholder"><div class="ph-icon">▮▮▮</div>NO CLIP LOADED</div>`;
      ...
      slot.innerHTML = `
        <div class="reveal">
          ...
          <button class="reveal-toggle" type="button">Show Findings</button>
      ...
      const toggle = slot.querySelector(".reveal-toggle");
      ...
      toggle.addEventListener("click", () => {
        ...
        toggle.textContent = showing ? "Show Findings" : "Hide Findings";
      });
